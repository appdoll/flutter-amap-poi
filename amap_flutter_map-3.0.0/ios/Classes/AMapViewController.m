...
- (void)setMethodCallHandler {
   ...
   [self.channel addMethodName:@"map#searchPOI" withHandler:^(FlutterMethodCall * _Nonnull call, FlutterResult  _Nonnull result) {
        NSString *approvalNumber = [weakSelf.mapView mapContentApprovalNumber];
        NSString *keyword = call.arguments[@"keyword"];
        NSString *city = call.arguments[@"city"];
        NSLog(@"searchPOI args from dart %@-%@",keyword,city);
...
- (void)onPOISearchDone:(AMapPOISearchBaseRequest *)request response:(AMapPOISearchResponse *)response
{
    ...
    [response.pois enumerateObjectsUsingBlock:^(AMapPOI *obj, NSUInteger idx, BOOL *stop) {
      NSDictionary *jsonObjc = [AMapJsonUtils jsonObjectFromModel:obj];
        NSMutableDictionary *dict = [NSMutableDictionary dictionaryWithDictionary:jsonObjc];
        [poiAnnotations addObject:dict];
    ...
    NSData *data=[NSJSONSerialization dataWithJSONObject:poiAnnotations options:NSJSONWritingPrettyPrinted error:nil];
    NSString *jsonStr=[[NSString alloc]initWithData:data encoding:NSUTF8StringEncoding];


    // 4. 将末尾逗号换成结束的]
    [jsonString replaceCharactersInRange:range withString:@"]"];
   // NSLog(@"打印poi结构：%@",jsonString);
    [_channel invokeMethod:@"map#onPOISearchDone" arguments:@{@"pois":jsonStr}];
}
