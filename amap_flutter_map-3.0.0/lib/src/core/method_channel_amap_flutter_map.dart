...
class MethodChannelAMapFlutterMap implements AMapFlutterPlatform {
  ...
  Stream<MapPoiSearchDoneEvent> onPoiSearchDone({required int mapId}) {
    return _events(mapId).whereType<MapPoiSearchDoneEvent>();
  }
  ...
  Future<dynamic> _handleMethodCall(MethodCall call, int mapId) async {
    switch (call.method) {
      ...
      case 'map#onPoiTouched':
        ...
        break;
      case 'map#onPOISearchDone':
        try {
          _mapEventStreamController.add(
              MapPoiSearchDoneEvent(mapId, call.arguments['pois']));
        } catch (e) {
          print('map#onPOISearchDone error===>' + e.toString());
        }
        break;
    }
  }
  ...
  Future<dynamic> searchPOI({
    ...
    return channel(mapId)
        .invokeMethod<dynamic>('map#searchPOI', <String, dynamic>{
          "keyword":keyword,
          "city":city
    });
  }
