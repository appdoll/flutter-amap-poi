...
class AMapController {
  ...
  void _connectStreams(int mapId) {
    ...
    if (_mapState.widget.onPoiSearchDone != null) {
      _methodChannel
          .onPoiSearchDone(mapId: mapId)
          .listen(((MapPoiSearchDoneEvent e) => _mapState.widget.onPoiSearchDone!(e.value)));
    }

    _methodChannel.onMarkerTap(mapId: mapId).listen((MarkerTapEvent e) => _mapState.onMarkerTap(e.value));

    _methodChannel.onInfoWindowTap(mapId: mapId).listen((InfoWindowTapEvent e) => _mapState.onInfoWindowTap(e.value));
  ...
  Future<dynamic> searchPOI({String? keyword, String? city}) {
    return _methodChannel.searchPOI(mapId: mapId, keyword: keyword, city: city);
  }
  /// 清空缓存
